        ...
        <div className="flex justify-between items-center h-16">
          ...
          <div className="hidden md:flex space-x-8 items-center">
            {navLinks.map((nav, index)=>(
               <NavLink to={nav.href} key={index} className={({ isActive }) =>isActive ?"text-gray-900":" text-gray-700 hover:text-gray-900"}>
                {nav.name}
                </NavLink>
            ))}
      ...
      {mobileMenuOpen && (
        <div className="md:hidden bg-white border-t">
          <div className="px-2 pt-2 pb-3 space-y-1">
            {navLinks.map((nav, index)=>(
               <NavLink to={nav.href} key={index} className={({ isActive }) => `${isActive ?"text-gray-900":" text-gray-700 hover:text-gray-900"} block w-full text-left px-3 py-2  `}>
                {nav.name}
                </NavLink>
            ))}
